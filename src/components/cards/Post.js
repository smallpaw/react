...
class Post extends Component {
	constructor(props) {
		...
		this.renderLikeAndCommentCount = this.renderLikeAndCommentCount.bind(
			this
		);
	}
	...
	renderComments() {
		return this.props.data.comments.map((item, index) => {
			return (
				<CommentWithLikes data={item} key={index}></CommentWithLikes>
			);
		});
	}
		...
		return (
			<Card className={this.props.className}>
				<CardBody>
					...
					<div
						className="mt-5 remove-last-border"
